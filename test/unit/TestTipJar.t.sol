...
contract TestTipJar is Test {
    ...
    function testTipUserIsSuccess() public registered {
        console.log(
            "Contract balance before tip: ",
            tipJar.getContractBalance()
        );
    ...
    function testWithdrawAllTransferFails() public registered hasBalance {
        //reset the contract balance to zero
        vm.deal(address(tipJar), 0);

        uint256 initialUserBalance = tipJar.getUserBalance(USER);

        vm.prank(USER);
        vm.expectRevert(TipJar.TipJar__TransferFailed.selector);
        tipJar.withdrawAllTips(USER);

        uint256 userBalance = tipJar.getUserBalance(USER);
        assertEq(userBalance, initialUserBalance);
    }

    function testWithdrawPartialTransferFails() public registered hasBalance {
        //reset the contract balance to zero
        vm.deal(address(tipJar), 0);
    ...
    function testUserCannotWithDrawPartialAnotherUsersFunds()
        public
        registered
        hasBalance
    {
    ...
    function testUserCannotWithdrawAllIfNotRegistered() public {
        vm.prank(USER);
        vm.expectRevert(TipJar.TipJar__UserMustBeRegistered.selector);
        tipJar.withdrawAllTips(USER);
    }

    function testUserCannotWithdrawPartialIfNotRegistered() public {
        vm.prank(USER);
        vm.expectRevert(TipJar.TipJar__UserMustBeRegistered.selector);
        tipJar.withdrawPartial(USER, TIP_AMOUNT);
    }

    function testUserCannotWithdrawPartialMoreThanTheirBalance()
        public
        registered
        hasBalance
    {
        vm.prank(USER);
        vm.expectRevert(TipJar.TipJar__UserBalanceNotSufficient.selector);
        tipJar.withdrawPartial(USER, TIP_AMOUNT * 2);
    }
    ...
    function testUserCantUnregisterWithBalance() public registered hasBalance {
        vm.prank(USER);
        vm.expectRevert(
            TipJar.TipJar__UserCannotUnregisterWithUnredeemedFunds.selector
        );
        tipJar.unregisterUser(USER);
    }
    ...
    function testUserCantUnregisterAnotherUser() public registered {
        vm.prank(NONREG_USER);
        vm.expectRevert(
            TipJar.TipJar__UserCanOnlyUnregisterThemselves.selector
        );
        tipJar.unregisterUser(USER);
    }
